"""Temperature modulation injection method."""
...
class TemperatureModulation:
    """Quantum temperature modulation.
    ...
    def modulate(
        ...
        Args:
            ...
            config: Sampler configuration (uses temp_modulation_beta, sample_count,
                population_mean, population_std, injection_verbose).
            beta_override: If provided, use this instead of config.temp_modulation_beta.

        Returns:
            Modulated temperature, clamped to >= 0.01. Returns input unchanged
            if beta == 0 or entropy is unavailable.
        """
        beta = beta_override if beta_override is not None else config.temp_modulation_beta
        if beta == 0.0:
            return temperature

        # Preserve zero/negative temperature (greedy mode).
        if temperature <= 0.0:
            return temperature
        ...
        try:
            ...
            _logger.warning("TemperatureModulation: entropy unavailable, skipping modulation")
            return temperature

        if not raw_bytes:
            _logger.warning("TemperatureModulation: empty entropy payload, skipping modulation")
            return temperature
        ...
        if config.injection_verbose:
            _logger.debug(
                "TemperatureModulation: beta=%.4f u=%.6f original=%.4f new=%.4f",
                beta,
                u,
                temperature,
                new_temp,
