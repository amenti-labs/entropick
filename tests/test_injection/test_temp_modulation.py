"""Tests for TemperatureModulation injection method."""
...
class TestTemperatureModulation:
    """Tests for TemperatureModulation.modulate()."""
    ...
    def test_modulate_changes_temperature_when_enabled(
        ...
        result = TemperatureModulation.modulate(0.7, source, config)
    ...
    def test_modulate_never_below_minimum(
        ...
        config = QRSamplerConfig(
            _env_file=None,  # type: ignore[call-arg]
            temp_modulation_beta=2.0,
        )
        # Use a very small starting temperature to stress the clamp.
        result = TemperatureModulation.modulate(0.02, source, config)
        assert result >= 0.01
    ...
    def test_modulate_range_bounded_by_beta(self) -> None:
        ...
        config = QRSamplerConfig(
            _env_file=None,  # type: ignore[call-arg]
            temp_modulation_beta=beta,
        )
        # Run many samples to check the range empirically.
        results = []
        for seed in range(100):
            src = MockUniformSource(seed=seed)
            results.append(TemperatureModulation.modulate(temperature, src, config))
    ...
    def test_modulate_handles_entropy_unavailable(self) -> None:
        ...
        result = TemperatureModulation.modulate(0.7, failing_source, config)
        assert result == pytest.approx(0.7)
    ...
    def test_modulate_handles_empty_entropy_payload(self) -> None:
        ...
        result = TemperatureModulation.modulate(0.7, empty_source, config)
        assert result == pytest.approx(0.7)
    ...
    def test_modulate_reproducible_with_same_seed(self) -> None:
        ...
        result_a = TemperatureModulation.modulate(0.7, source_a, config)
        result_b = TemperatureModulation.modulate(0.7, source_b, config)
